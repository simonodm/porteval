...
import { useGetAllKnownCurrenciesQuery, useGetLatestExchangeRatesQuery, useUpdateCurrencyMutation } from '../../redux/api/currencyApi';
                        ...
                        <select id="currency-select" onChange={(e) => onCurrencyChange(e.target.value)}>
                            {
                                currencies.data?.map(currency =>
                                    <option selected={currency.code === selectedCurrency?.code} value={currency.code}>{currency.code} ({currency.symbol})</option>)
                            }
                        </select>
                        <button role="button" className="btn btn-primary btn-sm" onClick={onSave}>Save</button>
                    </div>
                            ...
                            <tbody>
                                {
                                    exchangeRates.data?.map(exchangeRate => 
                                        <tr>
                                            ...
                                            <td>{getPriceString(exchangeRate.exchangeRate, currencies.data?.find(c => c.code === exchangeRate.currencyToCode)?.symbol)}</td>
                                        </tr>)
                                }
                            </tbody>
                        </table>
