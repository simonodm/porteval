...
public class TransactionsController : PortEvalControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<TransactionDto>>> GetTransactions(
        [FromQuery] TransactionFilters transactionFilters, [FromQuery] DateRangeParams dateRange)
    {
    ...
    public async Task<ActionResult<TransactionDto>> PostTransaction([FromBody] TransactionDto createRequest)
    {
        ...
        return GenerateActionResult(createdTransaction, nameof(GetTransaction),
            new { transactionId = createdTransaction.Response.Id });
    }

    // PUT api/transactions/3
    [HttpPut("{transactionId}")]
    public async Task<ActionResult<TransactionDto>> PutTransaction(int transactionId,
