...
public class InstrumentSplitsController : PortEvalControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<InstrumentSplitDto>> PostInstrumentSplit(int instrumentId,
        [FromBody] InstrumentSplitDto splitData)
    {
        ...
        return GenerateActionResult(createdSplit, nameof(GetInstrumentSplit),
            new { instrumentId, splitId = createdSplit.Response.Id });
    }

    // PUT instruments/5/splits/3
    [HttpPut("{splitId}")]
    public async Task<ActionResult<InstrumentSplitDto>> PutInstrumentSplit(int instrumentId,
        [FromBody] InstrumentSplitDto splitData)
    {
        if (instrumentId != splitData.InstrumentId)
            return BadRequest($"Query parameter {nameof(instrumentId)} and body instrument ID don't match.");
