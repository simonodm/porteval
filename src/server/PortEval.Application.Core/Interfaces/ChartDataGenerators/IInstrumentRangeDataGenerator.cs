...
public interface IInstrumentRangeDataGenerator : IDisposable
{
    ...
    ///     Each subsequent call to this method will return the data for the date range following the one returned by the
    ///     previous call until
    ///     there is no more data to generate, in which the <see cref="IsFinished" /> will return <c>true</c>.
    /// </summary>
    /// <returns>
    ///     A <see cref="InstrumentPriceRangeData" /> instance containing all the data needed to calculate metrics in the
    ///     next date range.
    /// </returns>
