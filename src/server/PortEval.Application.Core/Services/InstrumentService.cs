...
public class InstrumentService : IInstrumentService
{
    ...
    public InstrumentService(IInstrumentRepository instrumentRepository, ICurrencyRepository currencyRepository,
        IExchangeRepository exchangeRepository, IInstrumentQueries instrumentDataQueries,
        IExchangeQueries exchangeDataQueries, IInstrumentPriceService priceService,
        IInstrumentProfitCalculator profitCalculator, IInstrumentPerformanceCalculator performanceCalculator,
        IInstrumentChartDataGenerator chartDataGenerator,
        ICurrencyExchangeRateService exchangeRateService, ICurrencyConverter currencyConverter)
    {
    ...
    /// <inheritdoc />
    public async Task<OperationResponse<IEnumerable<EntityChartPointDto>>> ChartInstrumentPerformanceAsync(
        int instrumentId,
    ...
    /// <inheritdoc />
    public async Task<OperationResponse<IEnumerable<EntityChartPointDto>>> ChartInstrumentAggregatedProfitAsync(
        int instrumentId,
        DateRangeParams dateRange, AggregationFrequency frequency, string currencyCode = null)
    {
        var instrument = await _instrumentDataQueries.GetInstrumentAsync(instrumentId);
    ...
    /// <inheritdoc />
    public async Task<OperationResponse<IEnumerable<EntityChartPointDto>>> ChartInstrumentAggregatedPerformanceAsync(
        int instrumentId,
        DateRangeParams dateRange, AggregationFrequency frequency)
    {
        var instrument = await _instrumentDataQueries.GetInstrumentAsync(instrumentId);
    ...
    public async Task<OperationResponse<InstrumentDto>> CreateInstrumentAsync(InstrumentDto options)
    {
        ...
        var instrument = Instrument.Create(options.Name, options.Symbol,
            string.IsNullOrEmpty(options.Exchange) ? null : options.Exchange,
