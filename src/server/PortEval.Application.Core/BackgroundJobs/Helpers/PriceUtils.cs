...
internal static class PriceUtils
{
    ...
    /// <summary>
    ///     Fills price ranges of retrieved prices to maintain the desired intervals even in the absence of exchange price data
    ///     (e. g. during times when
    ///     the exchange is closed). For each missing time T it creates a new price point with the last available price before
    ///     T.
    /// </summary>
    ...
    public static IEnumerable<PricePoint> FillMissingRangePrices(IEnumerable<PricePoint> fetchedPrices,
        Func<DateTime, TimeSpan> intervalFunction,
    ...
    public static async Task<decimal> GetConvertedPricePointPrice(
    ...
    private static bool RangeShouldSplitAtInterval(TimeRange range, TimeSpan intervalCutoff, TimeSpan targetInterval,
        DateTime baseTime)
    {
        var timeDifferentFrom = baseTime - range.From;
        var timeDifferenceTo = baseTime - range.To;
        return timeDifferentFrom > intervalCutoff && timeDifferenceTo < intervalCutoff &&
               range.To >= baseTime - intervalCutoff + targetInterval;
    }
}