...
public class DataImportJob : IDataImportJob
{
    ...
    ///     Initializes the job.
    /// </summary>
    public DataImportJob(IServiceProvider serviceProvider, IDataImportRepository importRepository,
        IFileSystem fileSystem, ILoggerFactory loggerFactory, INotificationService notificationService)
    {
        ...
        _csvConfig = new CsvConfiguration(CultureInfo.InvariantCulture)
        {
            ...
            ReadingExceptionOccurred = args =>
            {
                ...
                _parsingErrors.Add(new RawRowErrorLogEntry(context.Parser.Record,
                    $"Failed to parse value \"{context.Parser[context.Reader.CurrentIndex]}\""));
                return false;
            }
        };
    }
    ...
    public async Task RunAsync(Guid importId, string inputFileName, string logPath)
    {
        ...
        catch (CsvHelperException ex)
        {
            ...
            importEntry.ChangeStatus(ImportStatus.Error,
                $"Error: failed to process received data as {importEntry.TemplateType}.");
        }
    ...
    private void SaveErrorLog<T>(IEnumerable<ProcessedRowErrorLogEntry<T>> processedErrorLog,
        IEnumerable<RawRowErrorLogEntry> parsingErrorLog, string filename)
    {
