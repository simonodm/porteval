...
/// <summary>
///     RapidAPI Mboum API client supporting retrieval of instrument prices.
/// </summary>
public class MboumApi : DataSource
{
    private const string HistoricalDataBaseUrl = @"https://mboum-finance.p.rapidapi.com/hi/history";
    private const string LatestDataBaseUrl = @"https://mboum-finance.p.rapidapi.com/qu/quote";
    ...
    public async Task<Response<PricePoint>> ProcessAsync(LatestInstrumentPriceRequest request)
    {
        ...
        return new Response<PricePoint>
        {
            StatusCode = response.StatusCode,
            ErrorMessage = response.ErrorMessage,
            Result = response.Result != null && response.Result.Any()
                ? new PricePoint
                {
    ...
    {
        var response = await GetHistoricalDataAsync(request.Symbol, "3mo", true);
    ...
    private async Task<Response<MboumHistoricalDataResponse>> GetHistoricalDataAsync(string ticker, string interval,
        bool downloadSplits = false)
    {
        var urlBuilder = new QueryUrlBuilder(HistoricalDataBaseUrl);
        ...
        var headers = new Dictionary<string, string>
        {
            ...
            { "X-RapidAPI-Host", new Uri(HistoricalDataBaseUrl).Host }
        };

        var result =
            await HttpClient.GetJsonAsync<MboumHistoricalDataResponse>(urlBuilder.ToString(), Configuration.RateLimiter,
                headers);

        return result;
    }

    private PricePoint ConvertHistoricalDataItemToPricePoint(MboumHistoricalDataItem item, string ticker,
        string currency)
    {
    ...
    {
        var urlBuilder = new QueryUrlBuilder(LatestDataBaseUrl);
        urlBuilder.AddQueryParam("symbol", ticker);

        var headers = new Dictionary<string, string>
        {
            { "X-RapidAPI-Key", Configuration.Credentials.Token },
            { "X-RapidAPI-Host", new Uri(LatestDataBaseUrl).Host }
        };

        var result =
            await HttpClient.GetJsonAsync<IEnumerable<MboumQuoteDataResponse>>(urlBuilder.ToString(),
                Configuration.RateLimiter, headers);
