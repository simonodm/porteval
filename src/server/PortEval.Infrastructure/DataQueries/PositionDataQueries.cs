...
{
    public class PositionDataQueries : IPositionDataQueries
    {
        private readonly IDbConnectionCreator _connectionCreator;

        public PositionDataQueries(IDbConnectionCreator connectionCreator)
        {
        ...
        public async Task<PositionDto> GetPosition(int positionId)
        {
            using var connection = _connectionCreator.CreateConnection();
            var query = @"SELECT *, TrackingInfo_LastUpdate as LastPriceUpdate FROM dbo.Positions
                          LEFT JOIN (
