...
public class InstrumentQueries : IInstrumentQueries
{
    ...
    public InstrumentQueries(PortEvalDbConnectionCreator connectionCreator)
    {
    ...
    /// <inheritdoc />
    public async Task<IEnumerable<InstrumentPriceDto>> GetInstrumentPricesPageAsync(
        int instrumentId,
        DateTime from,
        DateTime to,
    ...
    public async Task<InstrumentSplitDto> GetInstrumentSplitAsync(int instrumentId, int splitId)
    {
        ...
        return await connection.QueryFirstOrDefaultAsync<InstrumentSplitDto>(query,
            new { InstrumentId = instrumentId, SplitId = splitId });
    }
