...
public class InstrumentSplitsControllerTests
{
    ...
    private readonly Mock<IInstrumentSplitService> _instrumentSplitService;
    ...
    public async Task GetInstrumentSplits_ReturnsSplitsOfAnInstrument_WhenInstrumentExists()
    {
        ...
        _instrumentSplitService
            ...
            .ReturnsAsync(
                OperationResponseHelper.GenerateSuccessfulOperationResponse<IEnumerable<InstrumentSplitDto>>(new[]
                    { split }));
        ...
        Assert.Collection(result.Value, s => { Assert.Equal(split, s); });
    }
    ...
    public async Task PutInstrumentSplit_ReturnsBadRequest_WhenQueryParamInstrumentIdAndBodyInstrumentIdDontMatch()
    {
        ...
        _instrumentSplitService.Verify(m => m.UpdateSplitAsync(It.IsAny<int>(), It.IsAny<InstrumentSplitDto>()),
            Times.Never());
        Assert.IsAssignableFrom<BadRequestObjectResult>(result.Result);
    }
}