...
{
    private readonly Mock<ICsvExportService> _csvService;
    ...
    public async Task GetPortfoliosExport_ReturnsCsvFileWithPortfolioData()
    {
        ...
        _csvService
            ...
            .Returns(OperationResponseHelper.GenerateSuccessfulOperationResponse(
                Encoding.UTF8.GetBytes($"{portfolios.First().Id}")));
    ...
    public async Task GetPositionsExport_ReturnsCsvFileWithPositionsData()
    {
        ...
        _csvService
            ...
            .Returns(OperationResponseHelper.GenerateSuccessfulOperationResponse(
                Encoding.UTF8.GetBytes($"{positions.First().Id}")));
    ...
    public async Task GetTransactionsExport_ReturnsCsvFileWithTransactionsData()
    {
        ...
        _csvService
            ...
            .Returns(OperationResponseHelper.GenerateSuccessfulOperationResponse(
                Encoding.UTF8.GetBytes($"{transactions.First().Id}")));

        var sut = _fixture.Build<CsvExportController>().OmitAutoProperties().Create();

        var result = await sut.GetTransactionsExport(dateRange);

        _transactionService.Verify(m => m.GetTransactionsAsync(It.IsAny<TransactionFilters>(), dateRange),
            Times.Once());
    ...
    public async Task GetInstrumentExport_ReturnsCsvFileWithInstrumentsData()
    {
        ...
        _csvService
            ...
            .Returns(OperationResponseHelper.GenerateSuccessfulOperationResponse(
                Encoding.UTF8.GetBytes($"{instruments.First().Id}")));
    ...
    public async Task GetPricesExport_ReturnsCsvFileWithPricesData()
    {
        ...
        _csvService
            ...
            .Returns(OperationResponseHelper.GenerateSuccessfulOperationResponse(
                Encoding.UTF8.GetBytes($"{prices.First().Id}")));
