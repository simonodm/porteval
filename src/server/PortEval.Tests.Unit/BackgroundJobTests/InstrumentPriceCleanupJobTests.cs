...
namespace PortEval.Tests.Unit.BackgroundJobTests
{
    public class InstrumentPriceCleanupJobTests
    {
        ...
        public async Task Run_MaintainsOneDayIntervalForPricesOlderThan5Days()
        {
            ...
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 1)), Times.Never());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 2)), Times.Once());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 3)), Times.Never());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 4)), Times.Once());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 5)), Times.Never());
        }
        ...
        public async Task Run_MaintainsOneHourInterval_ForPricesInThePastFiveDays()
        {
            ...
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 2)), Times.Once());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 3)), Times.Never());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 4)), Times.Never());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 5)), Times.Once());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 6)), Times.Never());
        }
        ...
        public async Task Run_MaintainsFiveMinuteIntervalForPricesInThePastDay()
        {
            ...
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 1)), Times.Never());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 2)), Times.Never());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 3)), Times.Once());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 4)), Times.Never());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 5)), Times.Never());
            _priceRepository.Verify(c => c.Delete(It.Is<InstrumentPrice>(p => p.InstrumentId == instruments[0].Id && p.Id == 6)), Times.Once());
        }
    }
}
