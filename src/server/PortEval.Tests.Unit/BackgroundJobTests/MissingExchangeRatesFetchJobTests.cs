...
public class MissingExchangeRatesFetchJobTests
{
    ...
    private readonly Mock<ICurrencyExchangeRateRepository> _exchangeRateRepository;
    ...
    public async Task Run_ImportsExchangeRatesRetrievedFromPriceFetcher()
    {
        ...
        var firstTargetCurrency = new Currency("EUR", "European Euro", "€");
        var secondTargetCurrency = new Currency("CZK", "Czech Koruna", "Kč");
        ...
        _exchangeRateRepository
            .Setup(m => m.BulkUpsertAsync(It.Is<IList<CurrencyExchangeRate>>(list =>
                list.Count == 2 &&
                list.Any(
                    er => er.Time == time && er.CurrencyToCode == firstTargetCurrency.Code && er.ExchangeRate == 1m) &&
                list.Any(er =>
                    er.Time == time && er.CurrencyToCode == secondTargetCurrency.Code && er.ExchangeRate == 1m)
            )))
            .Returns(Task.Run(() => { }))
    ...
    public async Task Run_RetrievesOnlyMissingRanges()
    {
        ...
        var targetCurrency = new Currency("EUR", "European Euro", "€");
        var existingExchangeRates = new List<CurrencyExchangeRate>
        {
            new(1, existingExchangeRateTime, 1m, currency.Code, targetCurrency.Code)
        };
    ...
    {
        var currency = new Currency("USD", "US Dollar", "US$");
    ...
    private Mock<IFinancialDataFetcher> CreatePriceFetcherReturningExchangeRates(IFixture fixture,
        IEnumerable<ExchangeRates> exchangeRates)
    {
        var priceFetcher = fixture.Freeze<Mock<IFinancialDataFetcher>>();
        priceFetcher
            .Setup(m => m.GetHistoricalDailyExchangeRatesAsync(It.IsAny<string>(), It.IsAny<DateTime>(),
                It.IsAny<DateTime>()))
