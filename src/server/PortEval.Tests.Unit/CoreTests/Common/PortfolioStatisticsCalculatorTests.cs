...
public class PortfolioStatisticsCalculatorTests
{
    ...
    public void CalculateStatistics_ReturnsCorrectStatistics()
    {
        ...
        Assert.Equal(
            firstTransaction.Amount * firstInstrumentPriceDifference +
            secondTransaction.Amount * secondInstrumentPriceDifference, stats.TotalPerformance);
        Assert.Equal(
            firstTransaction.Amount * firstInstrumentPriceDifference +
            secondTransaction.Amount * secondInstrumentPriceDifference, stats.LastMonthPerformance);
        Assert.Equal(
            firstTransaction.Amount * firstInstrumentPriceDifference +
            secondTransaction.Amount * secondInstrumentPriceDifference, stats.LastWeekPerformance);
        Assert.Equal(
            firstTransaction.Amount * firstInstrumentPriceDifference +
            secondTransaction.Amount * secondInstrumentPriceDifference, stats.LastDayPerformance);
        Assert.Equal(
            firstTransaction.Amount * firstInstrumentPriceDifference +
            secondTransaction.Amount * secondInstrumentPriceDifference, stats.TotalProfit);
        Assert.Equal(
            firstTransaction.Amount * firstInstrumentPriceDifference +
            secondTransaction.Amount * secondInstrumentPriceDifference, stats.LastMonthProfit);
        Assert.Equal(
            firstTransaction.Amount * firstInstrumentPriceDifference +
            secondTransaction.Amount * secondInstrumentPriceDifference, stats.LastWeekProfit);
        Assert.Equal(
            firstTransaction.Amount * firstInstrumentPriceDifference +
            secondTransaction.Amount * secondInstrumentPriceDifference, stats.LastDayProfit);
    }
}