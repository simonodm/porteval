...
public class InstrumentServiceTests
{
    ...
    private readonly Mock<IExchangeQueries> _exchangeQueries;
    private readonly Mock<ICurrencyExchangeRateService> _exchangeRatesService;
    private readonly Mock<IExchangeRepository> _exchangeRepository;
    private readonly IFixture _fixture;
    private readonly Mock<IInstrumentChartDataGenerator> _chartDataGenerator;
    ...
    public async Task GetInstrumentsPageAsync_ReturnsInstrumentsPage()
    {
        ...
        var result = await sut.GetInstrumentsPageAsync(_fixture.Build<PaginationParams>().With(p => p.Page, 2)
            .With(p => p.Limit, takeCount).Create());
    ...
    public async Task GetInstrumentProfitAsync_ReturnsNotFound_WhenInstrumentDoesNotExist()
    {
        ...
        var sut = _fixture.Create<InstrumentService>();
        var profitResult =
            await sut.GetInstrumentProfitAsync(_fixture.Create<int>(), _fixture.Create<DateRangeParams>());
    ...
    public async Task GetInstrumentPerformanceAsync_ReturnsNotFound_WhenInstrumentDoesNotExist()
    {
        ...
        var performanceResult =
            await sut.GetInstrumentPerformanceAsync(_fixture.Create<int>(), _fixture.Create<DateRangeParams>());
    ...
    public async Task ChartInstrumentPricesAsync_ReturnsCorrectPriceChart_WhenInstrumentExists()
    {
        ...
        _chartDataGenerator
            .Setup(m => m.ChartPrices(It.IsAny<IEnumerable<InstrumentPriceDto>>(), It.IsAny<DateRangeParams>(),
                It.IsAny<AggregationFrequency>()))
            .Returns(chartData);

        var sut = _fixture.Create<InstrumentService>();
        var chartResult = await sut.ChartInstrumentPricesAsync(_fixture.Create<int>(),
    ...
    public async Task ChartInstrumentProfitAsync_ReturnsCorrectProfitChart_WhenInstrumentExists()
    {
        ...
        _chartDataGenerator
            .Setup(m => m.ChartProfit(It.IsAny<IEnumerable<InstrumentPriceDto>>(), It.IsAny<DateRangeParams>(),
                It.IsAny<AggregationFrequency>()))
            .Returns(chartData);

        var sut = _fixture.Create<InstrumentService>();
        var chartResult = await sut.ChartInstrumentProfitAsync(_fixture.Create<int>(),
            _fixture.Create<DateRangeParams>(), _fixture.Create<AggregationFrequency>());
    ...
    public async Task ChartInstrumentPerformanceAsync_ReturnsCorrectPerformanceChart_WhenInstrumentExists()
    {
        ...
        _chartDataGenerator
            .Setup(m => m.ChartPerformance(It.IsAny<IEnumerable<InstrumentPriceDto>>(), It.IsAny<DateRangeParams>(),
                It.IsAny<AggregationFrequency>()))
            .Returns(chartData);

        var sut = _fixture.Create<InstrumentService>();
        var chartResult = await sut.ChartInstrumentPerformanceAsync(_fixture.Create<int>(),
            _fixture.Create<DateRangeParams>(), _fixture.Create<AggregationFrequency>());
    ...
    public async Task ChartInstrumentAggregatedProfitAsync_ReturnsCorrectAggregatedProfitChart_WhenInstrumentExists()
    {
        ...
        _chartDataGenerator
            .Setup(m => m.ChartAggregatedProfit(It.IsAny<IEnumerable<InstrumentPriceDto>>(),
                It.IsAny<DateRangeParams>(), It.IsAny<AggregationFrequency>()))
            .Returns(chartData);

        var sut = _fixture.Create<InstrumentService>();
        var chartResult = await sut.ChartInstrumentAggregatedProfitAsync(_fixture.Create<int>(),
            _fixture.Create<DateRangeParams>(), _fixture.Create<AggregationFrequency>());
    ...
    [Fact]
    public async Task
        ChartInstrumentAggregatedPerformanceAsync_ReturnsCorrectAggregatedPerformanceChart_WhenInstrumentExists()
    {
        var chartData = _fixture.CreateMany<EntityChartPointDto>();

        _chartDataGenerator
            .Setup(m => m.ChartAggregatedPerformance(It.IsAny<IEnumerable<InstrumentPriceDto>>(),
                It.IsAny<DateRangeParams>(), It.IsAny<AggregationFrequency>()))
            .Returns(chartData);

        var sut = _fixture.Create<InstrumentService>();
        var chartResult = await sut.ChartInstrumentAggregatedPerformanceAsync(_fixture.Create<int>(),
            _fixture.Create<DateRangeParams>(), _fixture.Create<AggregationFrequency>());
