...
public class InstrumentQueriesTests
{
    ...
    {
        var queryResult = await _instrumentQueries.GetInstrumentPricesAsync(_appleInstrumentId,
    ...
    {
        var queryResult = await _instrumentQueries.GetInstrumentPricesPageCompressedAsync(_btcInstrumentId,
            DateTime.MinValue, DateTime.UtcNow, new PaginationParams
            {
                Limit = 300,
                Page = 1
            });
    ...
    {
        var queryResult = await _instrumentQueries.GetInstrumentPricesPageAsync(_btcInstrumentId, DateTime.MinValue,
            DateTime.UtcNow, new PaginationParams
            {
                Limit = 300,
                Page = 1
    ...
    {
        var queryResult =
            await _instrumentQueries.GetInstrumentPriceAsync(_btcInstrumentId, DateTime.UtcNow.AddDays(-1));
    ...
    {
        var queryResult =
            await _instrumentQueries.GetInstrumentPriceAsync(_btcInstrumentId, DateTime.UtcNow.AddDays(-7));
